module.exports = (client, message, box) => {
  ...
  if (message.content.toLowerCase().indexOf(client.config.prefix) !== 0) {
    ...
    // Run the command
    cmd.run(client, message, args, box);
  }
  ...
  // Setting up the getDayOfYear function
  function getDayOfYear(now) {
    // let daysInYear = (now.getFullYear() % 4 == 0 && now.getFullYear() % 100 != 0) || now.getFullYear() % 400 == 1 ? 366 : 365;
